...
def define_options():
    ...
    parser.add_argument(
        "--at_mobiles", dest="at_mobiles", help="待 @ 的手机号，多个用逗号分隔"
    )
    parser.add_argument(
        "--is_at_all",
        dest="is_at_all",
    ...
    parser.add_argument(
        "--msg", dest="msg", default="钉钉，让进步发生", help="要发送的消息内容"
    )
    return parser.parse_args()


def send_custom_robot_group_message(
    access_token, secret, msg, at_user_ids=None, at_mobiles=None, is_at_all=False
):
    """
    发送钉钉自定义机器人群消息
...
def main():
    ...
    send_custom_robot_group_message(
        options.access_token or ACCESS_TOKEN,
        options.secret or SECRET,
        options.msg,
